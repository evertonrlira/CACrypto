...
public abstract class VHCACryptoBase(string methodName, int blockSizeInBytes)
{
    private readonly string _name = methodName;
    private readonly int _keySizeInBytes = KeyBitsToRuleFactor * blockSizeInBytes;
    private readonly int _defaultBlockSizeInBytes = blockSizeInBytes;
    private readonly int _defaultBlockSizeInBits = 8 * blockSizeInBytes;
    ...
    public int MethodDefaultBlockSizeInBytes => _defaultBlockSizeInBytes;
    public int MethodDefaultBlockSizeInBits => _defaultBlockSizeInBits;
    public int MethodKeySizeInBytes => _keySizeInBytes;

    public Rule[] DeriveMainRulesFromKey(Span<int> keyBits, ToggleDirection direction)
    {
    ...
    public Rule[] DeriveBorderRulesFromKey(Span<int> keyBits, ToggleDirection direction)
    {
    ...
    public void BlockEncrypt(byte[] plainText, PermutiveCACryptoKey cryptoKey, byte[] ciphertext, int blockSize)
    {
    ...
    public void BlockEncrypt(byte[] initialLattice, Rule[] mainRules, Rule[] borderRules, byte[] finalLattice, int latticeSize)
    {
    ...
    protected void EvolveLatticeSlice(int[] preImage, Rule[] mainRules, Rule[] borderRules, int imageBorderLeftCellIdx, int[] image, int sliceStartInclusiveIdx, int sliceEndExclusiveIdx, int latticeSize)
    {
    ...
    private int[] SequentialEvolveLattice(int[] preImage, Rule[] mainRules, Rule[] borderRules, int imageBorderLeftCellIdx, int[] image, int latticeSize)
    {
        EvolveLatticeSlice(preImage, mainRules, borderRules, imageBorderLeftCellIdx, image, 0, latticeSize, latticeSize);
        return image;
    }

    private int[] ParallelEvolveLattice(int[] preImage, Rule[] mainRules, Rule[] borderRules, int imageBorderLeftCellIdx, int[] image, int latticeSize, int? threadCount = null)
    {
    ...
    public byte[] BlockDecrypt(byte[] cipherText, PermutiveCACryptoKey cryptoKey, byte[] plaintext, int blockSize)
    {
        Rule[] mainRules = DeriveMainRulesFromKey(cryptoKey.Bits, cryptoKey.Direction);
        Rule[] borderRules = DeriveBorderRulesFromKey(cryptoKey.Bits, cryptoKey.Direction);
    ...
    public byte[] BlockDecrypt(byte[] initialLattice, Rule[] mainRules, Rule[] borderRules, byte[] finalLattice, int latticeSize)
    {
        int latticeLengthInBits = 8 * latticeSize;
        var image = ArrayPool<int>.Shared.Rent(latticeLengthInBits);
        var preImage = ArrayPool<int>.Shared.Rent(latticeLengthInBits);
    ...
    private void PreImageCalculusBits(int[] image, Rule[] mainRules, Rule[] borderRules, int preImageBorderLeftCellIdx, int[] preImage, ToggleDirection toggleDirection, int latticeSize)
    {
