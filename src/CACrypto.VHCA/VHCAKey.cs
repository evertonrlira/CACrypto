...
{
    public VHCAKey(int[] keyBits, ToggleDirection toggleDirection) : base(Util.BinaryArrayToByteArray(keyBits), toggleDirection) { }

    public VHCAKey(byte[] keyBytes, ToggleDirection toggleDirection) : base(keyBytes, toggleDirection) { }

    public static VHCAKey GenerateRandomKey(int blockSize = VHCACrypto.BlockSizeInBytes, ToggleDirection? toggleDirection = null)
    {
        toggleDirection ??= Util.GetRandomToggleDirection();

        var keyBytes = Util.GetSecureRandomByteArray(VHCACrypto.KeyBitsToRuleFactor * blockSize);
